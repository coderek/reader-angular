...
import {paramsSelectFeed, paramsSelectEntry} from "../routes";
...
export class FeedEntriesComponent implements OnInit{
    ...
    ngOnInit() {
        this.route.params.map(p=>decodeURIComponent(p['feed'])).subscribe(val=> {
            this.store.dispatch(new SelectFeedAction(val));
        });
        this.route.params.map(p=>decodeURIComponent(p['open'])).subscribe(val=> {
